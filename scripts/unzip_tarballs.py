...
# Function to extract a tarball into a directory with the same name
def extract_tarball_to_named_dir(tarball_path, extract_base_dir):
    # Get the base name of the tarball (remove .tar.gz)
...
# Function to extract all tarballs in a source directory into directories with the same name
def extract_tarballs_in_directory(source_dir, target_dir):
    ...
    for filename in os.listdir(source_dir):
        if filename.endswith(".tar.gz"):
            ...
            extract_tarball_to_named_dir(tarball_path, target_dir)

# Example usage
import sys
source_directory = sys.argv[1] 
target_directory = sys.argv[2]
extract_tarballs_in_directory(source_directory, target_directory)
